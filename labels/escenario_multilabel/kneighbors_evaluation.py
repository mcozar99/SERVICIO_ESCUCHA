...
from config import corpus, model_label as model, percent, sentence_transformer, multilabel_dict, n_samples
...
def get_kneighbors_predictions():
    preds = pd.read_csv('./results/%s/labels/multilabel/%s_kneighbors_multilabel_predicts_samples%s_percent%s.txt'%(model, mode, n_samples, percent), names=['pred'],header=None)
...
print('KNEIGHBORS MULTILABEL EVALUATION')
discards_pred = get_kneighbors_discard_predictions()
evaluation(discards_pred['true'].tolist(), discards_pred['pred'].tolist(), '%s_kneighbors_multilabel_evaluation_%s_%s'%(mode, n_samples, percent))
final_pred = get_kneighbors_predictions()
evaluation(final_pred['true'].tolist(), final_pred['pred'].tolist(), '%s_kneighbors_final_multilabel_evaluation_%s_%s'%(mode, n_samples, percent))
